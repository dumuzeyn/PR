...
def acceleration_status() -> dict[str, object]:
    mode = os.environ.get("UZYRO_GPU", "auto").lower()
...
def minimum_pixels() -> int:
    return max(1, int(os.environ.get("UZYRO_GPU_MIN_PIXELS", "262144")))
...
def benchmark_acceleration(width: int = 2048, height: int = 1536, iterations: int = 3) -> dict[str, object]:
    ...
    destination[:, :, 3] = 255
    previous_threshold = os.environ.get("UZYRO_GPU_MIN_PIXELS")
    previous_mode = os.environ.get("UZYRO_GPU")
    os.environ["UZYRO_GPU_MIN_PIXELS"] = "1"
    os.environ["UZYRO_GPU"] = "force"
    try:
        ...
        if previous_threshold is None:
            os.environ.pop("UZYRO_GPU_MIN_PIXELS", None)
        else:
            os.environ["UZYRO_GPU_MIN_PIXELS"] = previous_threshold
        if previous_mode is None:
            os.environ.pop("UZYRO_GPU", None)
        else:
            os.environ["UZYRO_GPU"] = previous_mode
