...
class ActionWorkflowsMixin:
    ...
    def load_action_recording(self) -> None:
        path = filedialog.askopenfilename(filetypes=[("Действие UZYRO", "*.json")])
        if not path:
            return
        try:
    ...
    def run_action_file(self) -> None:
        path = filedialog.askopenfilename(
            filetypes=[("Действие UZYRO", "*.json"), ("Все файлы", "*.*")]
        )
        if not path:
            return
        try:
    ...
    def batch_action_file(self) -> None:
        action = filedialog.askopenfilename(filetypes=[("Действие UZYRO", "*.json")])
        if not action:
            return
    ...
    def _save_batch_queue(self) -> None:
        path = filedialog.asksaveasfilename(defaultextension=".json", filetypes=[("Очередь UZYRO", "*.json")])
        if path:
            self.batch_queue.save(path)

    def _load_batch_queue(self) -> None:
        path = filedialog.askopenfilename(filetypes=[("Очередь UZYRO", "*.json")])
        if path:
            try:
