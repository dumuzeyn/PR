...
class MenusToolsMixin:
    def _build_menu(self) -> None:
        ...
        destructive_filters = tk.Menu(filters, tearoff=False)
        filters.add_cascade(label="Разрушающие фильтры", menu=destructive_filters)
        destructive_filters.add_command(label="Размытие по Гауссу...", command=self.filter_blur)
        destructive_filters.add_command(label="Размытие в движении...", command=self.filter_motion_blur)
        destructive_filters.add_separator()
        destructive_filters.add_command(label="Контурная резкость...", command=self.filter_sharpen)
        destructive_filters.add_command(label="Умная резкость...", command=self.filter_smart_sharpen)
        destructive_filters.add_separator()
        destructive_filters.add_command(label="Добавить шум...", command=self.filter_noise)
        destructive_filters.add_command(label="Уменьшить шум...", command=self.filter_reduce_noise)
        destructive_filters.add_command(label="Цветовой контраст...", command=self.filter_high_pass)
        smart_filters = tk.Menu(filters, tearoff=False)
        filters.add_cascade(label="Смарт-фильтры", menu=smart_filters)
        smart_filters.add_command(label="Редактировать стек...", command=self.edit_layer_filters)
        smart_filters.add_separator()
        for kind in ("blur", "motion_blur", "unsharp_mask", "smart_sharpen", "noise", "reduce_noise", "high_pass"):
            smart_filters.add_command(label=f"Добавить: {FILTER_LABELS[kind]}", command=lambda value=kind: self.add_smart_filter(value))
        ...
        actions.add_command(label="Загрузить в редактор...", command=self.load_action_recording)
        actions.add_command(label="Редактировать запись...", command=self.show_action_editor)
    ...
    def _build_tool_options(self, parent: ttk.Frame) -> None:
        self.tool_options_panel = ToolOptionsPanel(
            ...
            brush_preset_name=self.brush_preset_name,
            brush_presets=self.brush_presets,
            apply_brush_preset=self.apply_brush_preset,
            ...
            import_custom_brush=self.import_custom_brush,
            ...
            shape_from_center=self.shape_from_center,
            ...
            clone_source_y=self.clone_source_y,
            clone_offset_x=self.clone_offset_x,
            clone_offset_y=self.clone_offset_y,
            clone_scale_x=self.clone_scale_x,
            clone_scale_y=self.clone_scale_y,
            clone_rotation=self.clone_rotation,
            clone_flip_horizontal=self.clone_flip_horizontal,
            ...
            patch_sample_all_layers=self.patch_sample_all_layers,
            apply_patch_preview=self.apply_patch_preview,
            open_clone_source_panel=self.open_clone_source_panel,
