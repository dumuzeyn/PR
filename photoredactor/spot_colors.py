...
def save_ase(path: str | Path, colors: Iterable[SpotColor], group_name: str = "PhotoRedactor") -> None:
    blocks: list[bytes] = []
...
def save_library(path: str | Path, colors: Iterable[SpotColor]) -> None:
    ...
    if target.suffix.lower() == ".ase":
        ...
        return
    target.write_text(
        json.dumps({"format": "PhotoRedactor spot colors", "version": 1, "colors": [color.to_dict() for color in values]}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
