...
def test_document_layer_and_basic_render_are_operational() -> None:
    document = Document.new(16, 12, (10, 20, 30, 255))
    overlay = np.full((5, 7, 4), (200, 40, 60, 128), dtype=np.uint8)
    document.add_layer("Overlay", overlay)
    document.layer.x, document.layer.y = 3, 2

    rendered = RenderEngine().render(document)

    assert rendered.shape == (12, 16, 4)
    assert rendered.dtype == np.uint8
    assert len(document.layers) == 2
    assert isinstance(document.layer, Layer)


@pytest.mark.ui
def test_application_initializes_and_closes_without_live_root(monkeypatch) -> None:
    monkeypatch.setattr(UZYROApp, "read_clipboard_image", staticmethod(lambda: None))
    app = UZYROApp()
    try:
        app.update_idletasks()
        assert app.startup_frame is not None
        assert app.document_manager.active is not None
...
def test_mask_fill_gradient_and_filter_stack_are_operational() -> None:
    document = textured_document()
    ...
    filtered = apply_filter_stack(layer.pixels, [
        {"type": "blur", "radius": 2},
