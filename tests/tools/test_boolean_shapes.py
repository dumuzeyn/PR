...
@pytest.mark.parametrize(
    ...
    ids=("union", "subtract", "intersect", "xor"),
)
def test_boolean_mode_has_correct_regions(mode: str, expected: tuple[bool, bool, bool]) -> None:
    document = overlapping_document()
    data = document.boolean_shape_data_with_lower(mode)
    assert data is not None
    mask = boolean_shape_mask(data, (80, 100))
    assert (mask[15, 15] > 0, mask[30, 40] > 0, mask[55, 60] > 0) == expected
