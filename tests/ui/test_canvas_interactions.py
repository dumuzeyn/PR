...
class CanvasInteractionUITests(ApplicationUITestCase):
    ...
    def test_new_document_recenters_after_previous_view(self) -> None:
        ...
        while self.app._initial_fit_after_id is not None and time.monotonic() < deadline:
            self.app.update()
        self.assertIsNone(self.app._initial_fit_after_id, "Canvas centering callback did not finish")
