...
class BrushWorkflowUITests(ApplicationUITestCase):
    def setUp(self) -> None:
        super().setUp()
    ...
    def _descendants(widget):
        for child in widget.winfo_children():
            yield child
            yield from BrushWorkflowUITests._descendants(child)
    ...
    def test_advanced_brush_settings_roundtrip_and_panel_tabs(self) -> None:
        self.app.brush_advanced.update({"angle": 37.0, "roundness": 0.42, "scatter": 1.25})
        self.app.brush_preset_name.set("Расширенный тест")
        self.app.save_brush_preset()
        self.app.brush_advanced.update({"angle": 0.0, "roundness": 1.0, "scatter": 0.0})
        self.app.apply_brush_preset()
        self.assertEqual(self.app.brush_advanced["angle"], 37.0)
        self.assertEqual(self.app.brush_advanced["roundness"], 0.42)
        self.assertEqual(self.app.brush_advanced["scatter"], 1.25)

        self.app.open_brush_engine_panel()
        self.app.update()
        dialogs = [widget for widget in self.app.winfo_children() if isinstance(widget, tk.Toplevel) and widget.title() == "Движок кисти"]
        ...
        dialogs[0].destroy()
        self.app.brush_presets.pop("Расширенный тест", None)
